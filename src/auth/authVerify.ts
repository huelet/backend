...
const verifyAuth = async (req: express.Request, res: express.Response) => {
    try {
        ...
        console.log(err);
        res.status(500).json({ response: `Error: ${err}`, errorCode: "0x11600", resolution: "Wait 15-20 minutes and try again. If it's not fixed, report a bug in the #bug-reports channel on discord" });
        return;
    }
}
